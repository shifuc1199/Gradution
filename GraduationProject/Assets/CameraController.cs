...
public class CameraController : MonoBehaviour
{
    ...
    Vector2 pos ;
    GameObject temp ;
    BaseGameObjectPool attack_effect_pool;
    private void Awake()
    {
        attack_effect_pool = new BaseGameObjectPool(effect_prefab);
    }
    public void Update()
    {
        ...
        if (isskill)
        {
            if(Input.GetKeyDown(KeyCode.Mouse0))
            {
                ...
                if (hit.collider)
                {
                    ...
                    temp = attack_effect_pool.Get(separated_body.transform.position+separated_body.transform.right*15, Quaternion.Euler(new Vector3(180-Quaternion.FromToRotation(Vector2.right, separated_body.transform.right).eulerAngles.z,90,0)),0.5f);
